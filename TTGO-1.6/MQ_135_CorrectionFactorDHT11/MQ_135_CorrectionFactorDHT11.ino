...
const unsigned long displayInterval = 1000; // Intervalo en milisegundos. Este es para 5 segundos
...
void loop() {
  ...
  MQ135.update(); // Update data, the arduino will read the voltage from the analog pin
  float PPM = MQ135.readSensor(false, cFactor); // Sensor will read PPM concentration using the model, a and b values set previously or from the setup
  
  // Comprobar si ha pasado suficiente tiempo desde la última vez que se mostraron los resultados
  if (millis() - lastDisplayTime >= displayInterval) {
    // Ha pasado suficiente tiempo, mostrar los resultados

    // Mostrar la cantidad de dióxido de carbono
    Serial.print("La concentración actual de dióxido de carbono en el ambiente es de ");
    Serial.print(PPM);
    Serial.println(" partes por millón (PPM).");
    ...
    if (!isnan(humidity_event.relative_humidity)) {
      Serial.print("La humedad relativa actual es del ");
      Serial.print(humidity_event.relative_humidity);
      Serial.println(" por ciento.");
    }

    // Guardar el tiempo actual para la próxima comprobación
